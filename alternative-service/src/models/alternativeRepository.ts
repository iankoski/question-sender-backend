...
function findAll(companyId: number, includeRemoved: boolean){
    ...
    return alternativeModel.findAll<IAlternativeModel>({where: {companyId: companyId, status:{[Op.not]: AlternativeStatus.REMOVED}}});
};
...
function findByQuestion(companyId: number, includeRemoved: boolean, questionId: number){
    ...
    //Excluindo os com status REMOVED
    return alternativeModel.findAll<IAlternativeModel>({where: {companyId: companyId, questionId: questionId, status:{[Op.not]: AlternativeStatus.REMOVED}}});
};
